...
########################################################    
    ...
    proband = datagenerate_person(proband_id, probandsex, proband_gen_dob, father_id, mother_id)

    father = datagenerate_person(father_id, "Male", parent_gen_dob, p_gpa_id, p_gma_id)
    mother = datagenerate_person(mother_id, "Female", parent_gen_dob, m_gpa_id, m_gma_id)
    
    p_gpa = datagenerate_person(p_gpa_id, "Male", grandparent_gen_dob)
    p_gma = datagenerate_person(p_gma_id, "Female", grandparent_gen_dob)

    m_gpa = datagenerate_person(m_gpa_id, "Male", grandparent_gen_dob)
    m_gma = datagenerate_person(m_gma_id, "Female", grandparent_gen_dob)
...
#######################################################
    ...
    siblings = make_children(num_children, proband_gen_dob, father_id, mother_id)
    fathers_siblings = make_children(num_children, parent_gen_dob, p_gpa_id, p_gma_id)
    mothers_siblings = make_children(num_children, parent_gen_dob, m_gpa_id, m_gma_id)
    proband_children = make_children(num_children, kids_gen_dob, proband_id, None)

    for person_id in siblings:
        siblings_children = make_children(num_children, kids_gen_dob, person_id, None)

    for person_id in fathers_siblings:
        fathers_siblings_children = make_children(num_children, proband_gen_dob, person_id, None)

    for person_id in mothers_siblings:
        mothers_siblings_children = make_children(num_children, proband_gen_dob, person_id, None)

    for person_id in proband_children:
        proband_grandchildren = make_children(num_children, grandkids_gen_dob, person_id, None)
