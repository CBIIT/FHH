/**
 * Edited by hendrikssonm on 9/5/2014.
 */
...
function transfer_save_xml() {

    $("#clone_xml").on("click", function () {
...
function add_personal_history() {
    ...
    add_personal_information_clone (patientPerson_tag);
    return subject_tag;
}

function add_personal_information_clone(patient_tag) {
    if (personal_information_clone == null) return;

    add_id(patient_tag, personal_information_clone.id);
    add_name(patient_tag, personal_information_clone.name);
    add_birthday(patient_tag, personal_information_clone.date_of_birth);
    add_gender(patient_tag, personal_information_clone.gender);
    add_all_ethnic_groups(patient_tag, personal_information_clone.ethnicity);
    add_all_races(patient_tag, personal_information_clone.race);
    add_clinical_observations(patient_tag,
        ...
        personal_information_clone.consanguinity,
        personal_information_clone["Health History"],
        null
    );
    add_relatives(patient_tag, personal_information_clone);
}
...
function add_relatives(tag, pi) {
    ...
    var i = 0;
    while (personal_information_clone['brother_' + i] != null) {
        add_individual_relative(tag, "Brother", "NBRO", personal_information_clone['brother_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['sister_' + i] != null) {
        add_individual_relative(tag, "Sister", "NSIS", personal_information_clone['sister_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['son_' + i] != null) {
        add_individual_relative(tag, "Son", "SON", personal_information_clone['son_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['daughter_' + i] != null) {
        add_individual_relative(tag, "Daughter", "DAU", personal_information_clone['daughter_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['maternal_aunt_' + i] != null) {
        add_individual_relative(tag, "Maternal Aunt", "MAUNT", personal_information_clone['maternal_aunt_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['maternal_uncle_' + i] != null) {
        add_individual_relative(tag, "Maternal Uncle", "MUNCLE", personal_information_clone['maternal_uncle_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['paternal_aunt_' + i] != null) {
        add_individual_relative(tag, "Paternal Aunt", "PAUNT", personal_information_clone['paternal_aunt_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['paternal_uncle_' + i] != null) {
        add_individual_relative(tag, "Paternal Uncle", "PUNCLE", personal_information_clone['paternal_uncle_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['maternal_cousin_' + i] != null) {
        add_individual_relative(tag, "Maternal Cousin", "MCOUSN", personal_information_clone['maternal_cousin_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['paternal_cousin_' + i] != null) {
        add_individual_relative(tag, "Paternal Cousin", "PCOUSN", personal_information_clone['paternal_cousin_' + i]);
        i++;
    }

    while (personal_information_clone['niece_' + i] != null) {
        add_individual_relative(tag, "Niece", "NIECE", personal_information_clone['niece_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['nephew_' + i] != null) {
        add_individual_relative(tag, "Nephew", "NEPHEW", personal_information_clone['nephew_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['grandson_' + i] != null) {
        add_individual_relative(tag, "Grandson", "GRDSON", personal_information_clone['grandson_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['granddaughter_' + i] != null) {
        add_individual_relative(tag, "GrandDaughter", "GRDDAU", personal_information_clone['granddaughter_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['maternal_halfbrother_' + i] != null) {
        add_individual_relative(tag, "Maternal Halfbrother", "HBRO", personal_information_clone['maternal_halfbrother_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['maternal_halfsister_' + i] != null) {
        add_individual_relative(tag, "Maternal Halfsister", "HSIS", personal_information_clone['maternal_halfsister_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['paternal_halfbrother_' + i] != null) {
        add_individual_relative(tag, "Paternal Halfbrother", "HBRO", personal_information_clone['paternal_halfbrother_' + i]);
        i++;
    }

    var i = 0;
    while (personal_information_clone['paternal_halfsister_' + i] != null) {
        add_individual_relative(tag, "Paternal Halfsister", "HSIS", personal_information_clone['paternal_halfsister_' + i]);
        i++;
    }
...
function save_document(site, doc) {
    ...
    if (personal_information_clone && personal_information_clone.name) {
        filename = personal_information_clone.name.replace(/ /g,"_") + "_Health_History.xml";
    }
