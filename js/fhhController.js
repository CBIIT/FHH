...
app.controller('tableController', ['$scope','$modalInstance','$timeout', function ($scope,$modalInstance,$timeout) {
	...
	$scope.collapse_status = {'isCollapsed':false,'collapse_text':$scope.translate("fhh_family_pedigree","collapse_table")};
		...
		for (key in STATICDISEASES) {
			if ($scope.collapse_status.isCollapsed) { // table has been collapsed
				if (initialDiseases.indexOf(STATICDISEASES[key])>-1) { // create normal disease column
					...
					dl.push({'code':d,'translatedDiseaseName':$scope.translate("diseases",d),'show':true})
				}	
				else {
					if (!otherDiseases) {
						dl.push({'code':'1','translatedDiseaseName':$scope.translate("fhh_family_pedigree","other_disease"),'show':true});
						otherDiseases = true;
					}
				}			
			}
	...
	$scope.personal_information = function() {
		...
		new_personal_information.push({'name':pi.name,'gender':pi.gender,'relationship':$scope.translate("fhh_js","self"),'is_alive':$scope.translate("fhh_family_pedigree","alive"),'Health History':self_health_history.health_history,'health_code_lookup':self_health_history.health_code_lookup})
	...
	$scope.createHealthHistory = function(health_history,health_code_lookup,history) {
		...
		return {'health_history':health_history,'health_code_lookup':health_code_lookup}
	};
			...
			$timeout(function() {
				var i = $scope.diagram.makeImageData();
					...
					var WindowObject = window.open("", "Table",
	    				"width="  + $("table.health_table").width() + ",height=" + $("table.health_table").height() + ",top=50,left=50,toolbars=no,scrollbars=yes,status=no,resizable=yes");
	...
	$scope.displayHiddenDiagram = function() {
		...
		var createTableHeaders = function() {
			 ...
			 var table = $(go.Panel, "Table",{defaultRowSeparatorStroke: "gray",defaultColumnSeparatorStroke: "gray"})
			 var row = $(go.Panel, "TableRow", {background: '#00008B',row:0});
			  row.add($(go.TextBlock, "Name",{stroke:"white", width:100,column: 0, font: "bold 10pt sans-serif", margin: 6,textAlign:"left"}));
			  row.add($(go.TextBlock, "Still Living",{stroke:"white", width:100,column: 1, font: "bold 10pt sans-serif", margin: 6,textAlign:"left"}));
			 for (key in $scope.disease_list) {
			   row.add($(go.TextBlock, $scope.disease_list[key].translatedDiseaseName,{stroke:"white", width:90,column: parseInt(key)+2, font: "bold 10pt sans-serif", margin: 6,textAlign:"left"}));
			 }
			 table.add(row);
			 
			 for (key in $scope.pi) {
			   row = $(go.Panel, "TableRow", {row:parseInt(key)+3});
			     row.add($(go.TextBlock, $scope.pi[key].name+"\n("+$scope.pi[key].relationship+")",{ width:100,column: 0, font: "bold 10pt sans-serif", margin: 6,textAlign:"left"}));
			     row.add($(go.TextBlock, $scope.pi[key].is_alive,{ width:100,column: 1, font: "bold 10pt sans-serif", margin: 6,textAlign:"left"}));
			   
			   for (col in $scope.disease_list) {
			     row.add($(go.TextBlock, $scope.lookupDisease($scope.pi[key],$scope.disease_list[col].code,true),{ width:90,column: parseInt(col)+2, font: "bold 10pt sans-serif", margin: 6,textAlign:"left"}));
			   } 
			   table.add(row)
			 }
