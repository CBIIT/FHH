...
function bind_personal_submit_button_action() {
	...
	$("#addPersonInformationSubmitButton").on("click", function () {
		...
		if ($("#personal_info_form_gender_male").prop('checked') == false &&
			...
			$("#personal_info_form_gender_other").prop('checked') == false) {
			if (!$("#invalid_gender_warning").length) {
				$('#personal_info_form_gender_other').next().after(
	...
	$("#addFamilyMemberSubmitButton").on("click", function () {
		...
		if (alive_flag == 'alive') {
			...
			if (new_disease && cause_of_death != null && cause_of_death_code != null && cause_of_death_code != 'not_picked') {
				...
				specific_health_issue = {
					"Disease Name": cod_temp,
...
function load_risk_links() {
	...
	$.getJSON("../risk/risks.json", function (data) {
		//debugger;
		$("#risk_section").empty();
		$("#risk_section").append("<div class='title-bar'>Check your familial risks for the following diseases:</div>")
		var table = $("<table id='risk_table' class='risk_table'>");
		
		add_risk_calculator_header_row(table);
		...
		$.each(data, function (index) {
			...
			if (data[index].status == 'active') {
				...
				if (data[index]['name.' + lng] == 'Heart Disease') {

					var new_row = $("<tr></tr>");

					
					var disease_td = $("<td></td>")
					disease_td.append(data[index]['name.' + lng]);
					new_row.append(disease_td);

					var heart_risk = main_test_heart();
					if (heart_risk) {

						new_row.append("<td><IMG alt='good_because_negative' src='../risk/image/triangle.gif' height='20'/></td>");
					}
					else {
						new_row.append("<td><IMG alt='good_because_negative' src='../risk/image/smiley.gif' height='20'/></td>");
					}

					var detail_td = $("<td style='text-align:center;border:1px solid #888; padding:2px;'>");
					var detail_link = $("<div class='risk_detail' href='" + data[index].link + "'>"); 
					detail_link.append("<img style='border:0' src='../images/rachet3.jpg' alt='risk assessment' title='Risk Assessment'>");
					detail_td.append(detail_link);
					new_row.append(detail_td);

					table.append(new_row);
				}
				else if (data[index]['name.' + lng] == 'Breast and Ovarian Cancer') {

					var new_row = $("<tr></tr>");

					var brca_risk = main_test_breast();
				...
				else if (data[index]['name.' + lng] == 'Colorectal Cancer') {

					var new_row = $("<tr></tr>");

					var crca_risk = main_test_crca();
				...
				else if (data[index]['name.' + lng] == 'Diabetes') {
					...
					if(personal_information['weight'] == "" || personal_information['height'] == "")
					{
						new_row.append("<td><IMG alt='good_because_negative' src='../risk/image/blue_waiting_sm.gif' height='20'/></td>");
					}
					else
					{
						debugger;
						var diabete_risk = main_test_diabete();
						if (diabete_risk) {
		...
		$("#risk_section").append(table);
		$("#risk_section").append("<br><div align='left'> <IMG alt='good_because_negative' src='../risk/image/triangle.gif' height='15'/>: possible risk increase based on the current information.  </div>");
		$("#risk_section").append("<div align='left'><IMG alt='good_because_negative' src='../risk/image/smiley.gif' height='15'/>: average risk based on the current information.   </div>");
		$("#risk_section").append("<div align='left'><IMG alt='good_because_negative' src='../risk/image/blue_waiting_sm.gif' height='15'/>: not assessed due to lack of required information.</div>");
		$("#risk_section").append("<div align='left'>Click on the Assessment Detail icon <img style='border:0' src='../images/rachet3.jpg' height='18' alt='risk assessment' title='Risk Assessment'> to perform the assessment.</div>");
		
		$("#risk_section").append("<div align='left'>Note: Overall Risk Assessment result may be changed after more information provided during the assessment.</div>");

		

		$("#risk_detail_window").empty();

		$(".risk_detail").on("click", function () {
			//debugger;
...
function add_risk_calculator_header_row(table) {
	var header_row = $("<tr></tr>");
	header_row.append("<th scope='col' class='nowrap'>Disease</th>");
	header_row.append("<th scope='col' abbr='Relationship' class='nowrap'>Overall Risk Assessment</th>");
	header_row.append("<th scope='col' abbr='Living' class='nowrap'>Assessment Detail</th>");
	header_row.append("");
...
function build_race_ethnicity_section(race_ethnicity, personal_flag) {
	...
	if (personal_flag) {
		...
		table.append($("<tr>")
			.append("<td colspan='3'><br><input name='person.consanguinity' value='true' tabindex='20' id='person_consanguinity' type='checkbox'/>&nbsp <b><label for='person_consanguinity'>" + $.t("fhh_js.consanguinity") + "</b>" + $.t("fhh_js.consanguinity_note") + "</label><br><br>"
				+ "</td>"));
	}
	table.append($("<tr>").append("<td colspan='2'><b>" + $.t("fhh_js.select_races_selectable") + "</b>" + $.t("fhh_js.multiple_races_selectable") + "</td>"));
	table.append($("<tr>")
		.append("<td style='width:150px;'>" + "<em>" + $.t("fhh_js.race") + "</em></td>")
		.append(race_checkboxes));
	table.append($("<tr id='asian_checkboxes'>")
		.append("<td>" + "<em>" + $.t("fhh_js.more_race") + "</em></td>")
		.append(asian_race_checkboxes));
	table.append($("<tr id='south_pacific_checkboxes'>")
		.append("<td>" + "<em>" + $.t("fhh_js.more_race") + "</em></td>")
		.append(south_pacific_race_checkboxes));
	table.append($("<tr>")
		.append("<td><br />" + "<em>" + $.t("fhh_js.ethnicity") + "</em></td>")
		.append(ethnicity_checkboxes));
	table.append($("<tr id='hispanic_checkboxes'>")
		.append("<td>" + "<em>" + $.t("fhh_js.more_ethnicity") + "</em></td>")
...
function clear_and_set_personal_health_history_dialog() {
	...
	if (personal_information.gender == "OTHER") $('#personal_info_form_gender_other').prop('checked', true);
	else $('#personal_info_form_gender_other').prop('checked', false);
